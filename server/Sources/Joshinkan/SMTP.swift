...
extension User {
    init(description: String) throws {
        ...
        if match.output.email == nil && match.output.name != nil {
            email = String(match.output.name!)
            name = nil
        } else {
...
struct Mail {
    ...
    func toPayload() -> String {
        assert(!to.isEmpty)
        return """
        To: \(to.map(\.description).joined(separator: ", "))
        From: \(from)
        Subject: \(subject)
...
struct UserData {
    ...
    static func curlRead(
        ...
        if userData.bytesRead >= userData.payload.count {
            return 0
        }
...
struct SMTP: SMTPClient {
    ...
    func send(_ mail: Mail) throws {
        ...
        let recipients = list2curl(mail.recipients.map(\.email))
